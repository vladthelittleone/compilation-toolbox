...
public class JavaSourceCompilerImplTest {
	...
	public void testCompileWithClassPath() throws Exception {
		...
		compilationUnit.addJavaSource("com.test.Foo", ""
				...
				+ "    }");
		javaSourceCompiler.compile(compilationUnit, "-classpath",
				"src/target/classes");
	...
	public void testPersistCompiledClasses() throws Exception {
		...
		JavaSourceCompiler.CompilationUnit compilationUnit = javaSourceCompiler
				.createCompilationUnit(compilationRoot);
		String javaSourceCode = "   " + "package com.test.foo;"
				+ "public class Foo {\n"
				+ "        public static void main(String [] args) {\n"
				+ "            System.out.println(\"Hello world\");\n"
	...
	/**
	 * This test has changed, initially when compilationRoot did not exist, this
	 * method did not persist
	 */
	public void testPersistCompiledClassesWithNonExistingRoot()
			throws Exception {
		JavaSourceCompiler javaSourceCompiler = new JavaSourceCompilerImpl();
		...
		{
			String javaSourceCode = "   " + "package com.test.foo.bar;"
					+ "public class Bar {\n"
					+ "        public static void main(String [] args) {\n"
					+ "            System.out.println(\"Hello world\");\n"
					+ "        }\n" + "}";
			compilationUnit.addJavaSource("com.test.foo.bar.Bar",
					javaSourceCode);
		}
		javaSourceCompiler.compile(compilationUnit);
		javaSourceCompiler.persistCompiledClasses(compilationUnit);
