...
public class SimpleClassLoaderTest {
	...
	public void testSimpleClassLoader() {
		SimpleClassLoader classLoader = new SimpleClassLoader(null,
				getRegistry(), new File("target/simple-compiler"));
	...
	public void testFindClassInClassPath() throws ClassNotFoundException {
		...
		Class<?> clazz = classLoader.findClass(SimpleClassLoaderTest.class
				.getName());
		Assert.assertNotNull(clazz.getName(),
				SimpleClassLoaderTest.class.getName());
	}
	...
	public void testFindResource() throws Exception {
		...
		String injectJarFile = TestHelper.getResourcePath("javax", "inject");
		classLoader.addClassPathEntry(injectJarFile);
		Assert.assertTrue(Collections.list(classLoader.findResources("com"))
				.size() > 0);
		Assert.assertFalse(Collections.list(
				classLoader.findResources("dummiesF")).size() > 0);
